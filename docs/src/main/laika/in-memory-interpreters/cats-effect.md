...
Wrap `run` in `IO` to lift the result into cats-effect:
...
val result: IO[List[Person]] = IO(program.run(Universe.zero))
```
...
val result: IO[List[Person]] =
  IO(service.createAndList(persons).run(Universe.zero))
```

Because the model has been validated against a real database via `MirraSuite`, this service test is behaviorally accurate — not just a mock that returns whatever you tell it to.

> To verify model against a real database, see [Test suite — munit + cats-effect](../data-layer-testing/munit.md).
